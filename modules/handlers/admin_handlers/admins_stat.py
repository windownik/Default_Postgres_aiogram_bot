...
async def create_stat_text():
    ...
    all_users_close = len(await data_b.read_by_name(name='id', id_name='status', id_data='close'))
    all_new_users = len(await data_b.read_all_by_date())
    active_users_7 = len(await data_b.read_all_by_date(days=7, data_column='activity'))
    active_users_1 = len(await data_b.read_all_by_date(days=1, data_column='activity'))
